...
--					WHEN b._date_of_birth_date BETWEEN DATEADD(day,-8,s.dob) AND DATEADD(day,8,s.dob) THEN 0.5
				...
				CASE WHEN LEN(b._mother_res_addr1) > 5 AND b._mother_res_addr1 IN (
...
--		Requiring same date of birth should speed things up.
			LEFT JOIN webiz.immunizations s ON b._date_of_birth_date = s.dob AND
				(( b.sex in (1,2) AND s.gender_code = CASE WHEN b.sex = 1 THEN 'M' WHEN b.sex = 2 THEN 'F' END )
					OR b.sex not IN (1,2) )
